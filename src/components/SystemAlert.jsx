...
const SystemAlert = ({ isOpen, onClose, title, message, type = 'info', onConfirm, confirmText = 'CONFIRM', cancelText = 'CANCEL', children }) => {
 ...
 {message && (
 <p className="text-sm md:text-base text-gray-300 mb-6 font-['Rajdhani'] leading-relaxed tracking-wide px-2">
 {message}
 </p>
 )}
 ...
 {onConfirm && (
 <button onClick={onClose} className="flex-1 py-3 bg-white/5 border border-white/10 rounded-xl text-xs md:text-sm md:text-base hover:bg-white/10 transition-all text-gray-400 hover:text-white tracking-[0.15em] font-bold active:scale-95">
 {cancelText}
 </button>
 )}
 ...
 className={`flex-1 py-3 ${current.bg} border ${current.border} rounded-xl text-xs md:text-sm md:text-base ${current.color} hover:bg-opacity-30 transition-all font-bold tracking-[0.15em] active:scale-95 relative overflow-hidden group`}
 >
