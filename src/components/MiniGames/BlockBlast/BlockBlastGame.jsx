...
const saveScore = async () => {
  try {
    // ดึง user ปัจจุบันจาก supabase (ต้องแน่ใจว่า user ล็อกอินแล้ว)
    const { data: { user } } = await supabase.auth.getUser();
    if (!user) return;

    // บันทึกคะแนนลงตาราง 'leaderboard'
    // username ให้ดึงจาก email หรือ profile แล้วแต่คุณเก็บไว้ที่ไหน
    const username = user.email.split('@')[0]; 
    
    await supabase.from('leaderboard').insert([
      { username: username, score: score, game_slug: 'block-blast' }
    ]);
  } catch (error) {
...
// 1. เพิ่ม parameter: comboCount เพื่อเอามาคำนวณเสียง
const playSound = (type, isMuted, comboCount = 0) => {
  ...
  if (type === 'grab') { 
    ...
    osc.type = 'square'; 
    // คำนวณ Pitch Shift: ยิ่ง Combo เยอะ เสียงยิ่งสูงขึ้น (เพิ่มทีละ 150Hz) 
    // ใช้ Math.min ล็อกไว้ที่ 10 คอมโบ กันเสียงแหลมทะลุปรอทจนปวดหู
...
const BlockBlastGame = () => {
  ...
  useEffect(() => {
    ...
    style.innerHTML = `
      ...
      /* บังคับ Canvas ให้โปร่งใสแน่นอน 100% */
      canvas { 
  ...
  const handleMouseUp = () => {
    ...
    if (placement.canPlace) {
      ...
      if (linesCleared > 0) {
        ...
        // คืนค่าเช็ค 3 แถวให้เป็น 3 เหมือนเดิมนะครับ (เผื่อคุณเทสเสร็จแล้ว)
        if (linesCleared >= 3) {
        ...
        // 2. โยนค่า nextCombo เข้าไปในฟังก์ชันเสียง
      ...
      if (!canPlaceAnyBlock({ grid: gameData.current.grid, availableBlocks: gameData.current.availableBlocks })) {
        ...
        saveScore(); // <--- เรียกฟังก์ชันนี้ตอนจบเกม
      }
    } else {
      const originalBlock = gameData.current.availableBlocks.find(b => b.id === activeBlock.id);
      if (originalBlock) originalBlock.active = true;
  ...
  return (
    ...
    <div className="relative flex flex-col items-center w-full h-[100dvh] overflow-hidden"
    ...
    >
      ...
      <div className="flex-1 w-full flex items-center justify-center px-2 pt-16 md:pt-0">
          <canvas 
            ...
            // เอา style={backgroundColor: '#08050f'} ออกไป
            // ใช้ will-change เพื่อแก้ Glitch แทนการใช้สีดำ
            style={{ 
