...
const ResetPasswordModal = ({ isOpen, onClose, userEmail, notify }) => {
 ...
 <h2 className="font-['Orbitron'] text-[#f0eaff] font-bold tracking-widest text-lg">RESET PASSWORD</h2>
 <p className="text-gray-400 text-xs md:text-sm md:text-base text-center mt-2">กรอกรหัส OTP ที่ได้รับทางอีเมลและตั้งรหัสผ่านใหม่</p>
 </div>
 ...
 className="w-full bg-black/50 border border-white/10 rounded-lg p-3 text-sm md:text-base text-[#f0eaff] focus:outline-none focus:border-[#d966ff]/50"
 />
 </div>

 {localError && (
 <div className="mb-4 text-center">
 <span className="text-red-400 text-xs md:text-sm md:text-base font-bold">{localError}</span>
 </div>
 )}
 ...
 disabled={loading}
 className="w-full py-3 bg-red-500/20 border border-red-500/50 rounded-lg text-red-400 text-sm md:text-base font-bold tracking-widest hover:bg-red-500/30 transition-all active:scale-95 disabled:opacity-50"
 >
