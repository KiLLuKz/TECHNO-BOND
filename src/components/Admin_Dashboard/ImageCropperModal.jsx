...
const ImageCropperModal = ({ 
 ...
 {description && (
 <p className="text-[#d966ff] mb-4 text-center text-sm md:text-base">
 {description}
 </p>
 )}
 {!imageSrc ? (
 ...
 <p className="text-gray-400 text-center px-4">คลิกเพื่อเลือกรูปภาพ (อัตราส่วน {aspectRatio === 1 ? '1:1' : '3:4'})</p>
 <input 
 type="file" 
 accept="image/*" 
 ...
 <label className="text-xs md:text-sm md:text-base text-[#99eedd] mb-2 block">ZOOM</label>
 <input
 type="range"
 value={zoom}
 min={1}
 ...
 className="px-4 py-2 rounded-lg text-sm md:text-base text-gray-400 hover:bg-white/10 transition-colors"
 >
 RESET
 </button>
 <button 
 ...
 className="flex items-center gap-2 px-6 py-2 bg-[#7ecfff]/20 hover:bg-[#7ecfff]/40 border border-[#7ecfff]/50 text-[#7ecfff] rounded-lg font-bold transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
 >
 {uploading ? (
 <span>UPLOADING...</span>
 ) : (
